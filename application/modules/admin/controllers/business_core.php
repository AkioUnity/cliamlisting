...
class Business_core extends CI_Controller {
	...
	public function deletepost($page='0',$id='',$confirmation='')
	{
		...
		if($confirmation=='')
		{
			...
			$info = '"'.$title.'"';
			$data['content'] = load_admin_view('confirmation_view',array('id'=>$id,'url'=>site_url('admin/business/deletepost/'.$page),'info'=>$info),TRUE);
			load_admin_view('template/template_view',$data);
			//end
		}
		else
	...
	public function sendnewsletter()
	{
		...
		$data['content'] 	= load_admin_view('business/newsletter_email_view',$value,TRUE);
		load_admin_view('template/template_view',$data);
	}
	//end
	...
	public function sendbulkemail($agent_id='0')
	{
		...
		else
		{
			...
			if($this->input->post('from')=='newsletter')
			redirect(site_url('admin/business/sendnewsletter'));
			else
	...
	public function deletelocation($page='0',$id='',$confirmation='')
	{
		...
		{
			//added on version 1.6
			$location = $this->business_model->get_location_by_id($id);
			$info = '"'.$location->name.'"';
			$data['content'] = load_admin_view('confirmation_view',array('id'=>$id,'url'=>site_url('admin/business/deletelocation/'.$page),'info'=>$info),TRUE);
			load_admin_view('template/template_view',$data);
			//end
		}
		else
	...
	public function remove_unused_gallery_img()
	{
		$file_name = $this->input->post('name');
		if($file_name!='')
		{
			if (!unlink('./uploads/gallery/'.$file_name))
			{
			  echo "no";
			}
			else
